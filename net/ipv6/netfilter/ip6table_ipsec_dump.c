...
static int nf_xfrm6_output_decode_cap_log(struct sk_buff *skb,
...
static int nf_xfrm6_input_decode_cap_log(struct sk_buff *skb)
{
	struct packet_type *ptype;
	struct packet_type *pt_prev = NULL;
	struct net_device *orig_dev;
...
{
	/* Check whether tcpdump hook exist. */
...
MODULE_DESCRIPTION("Hook for dump vowifi ipsec decoding pkts in tcpdump v6 files.");
