...
#define TRUSTY_LOG_SIZE (PAGE_SIZE * 4)
...
static DEVICE_ATTR(trusty_loglevel, 0660, trusty_loglevel_show, trusty_loglevel_store);
...
static DEVICE_ATTR(trusty_console, 0660, trusty_console_show, trusty_console_store);
...
static int trusty_log_probe(struct platform_device *pdev)
{
	...
	result = device_create_file(s->dev, &dev_attr_trusty_loglevel);
	if (result)
		dev_err(&pdev->dev, "failed to register device file\n");

	result = device_create_file(s->dev, &dev_attr_trusty_console);
	if (result)
		dev_err(&pdev->dev, "failed to register device file\n");
