...
struct cluster0_sensor cluster0_temp_sensor;
...
static int __sprd_get_max_temp(int *temp)
{
	...
	struct cluster0_sensor *psensor = &cluster0_temp_sensor;
	...
	for (i = 0; i < psensor->nsensor; i++) {
		cluster0_temp = max(cluster0_temp, sensor_temp[i]);
...
static int sprd_temp_sensor_read(struct sprd_thermal_zone *pzone, int *temp)
{
	...
	pr_debug("cluster0_sensor_id:%d, temp:%d\n", pzone->id, *temp);

	return ret;
}

struct thm_handle_ops sprd_cluster0_thm_ops = {
	.read_temp = sprd_temp_sensor_read,
};

static void sprd_ipa_info_copy(struct sprd_thermal_zone *pzone,
				struct cluster0_sensor *pcluster0_sensor)
{
	struct sprd_ipa_info *ipa_info = &pcluster0_sensor->ipa_info;
				...
				struct cluster0_sensor *pcluster0_sensor)
{
	...
	pcluster0_sensor->nsensor = count;
	for (i = 0; i < count; i++) {
		ret = of_property_read_string_index(np, "sensor-names",
			i, &pcluster0_sensor->sensor_names[i]);
		if (ret) {
	...
	sprd_ipa_info_parse_dt(np, &pcluster0_sensor->ipa_info);

	return ret;
}

static int sprd_cluster0_thm_probe(struct platform_device *pdev)
{
	int i = 0;
	int ret = 0, sensor_id = 0;
	struct sprd_thermal_zone *pzone = NULL;
	struct cluster0_sensor *pcluster0_sensor = &cluster0_temp_sensor;
	...
	ret = sprd_temp_sen_parse_dt(&pdev->dev, pcluster0_sensor);
	if (ret) {
		dev_err(&pdev->dev, "not found ptrips\n");
		return -EINVAL;
	}

	for (i = 0; i < pcluster0_sensor->nsensor; i++) {
		pcluster0_sensor->thm_zones[i] =
		thermal_zone_get_zone_by_name(
			pcluster0_sensor->sensor_names[i]);
		if (IS_ERR(pcluster0_sensor->thm_zones[i])) {
	...
	pr_info("sprd cluster0 sensor id %d\n", sensor_id);

	pzone->dev = &pdev->dev;
	pzone->id = sensor_id;
	pzone->ops = &sprd_cluster0_thm_ops;
	strlcpy(pzone->name, np->name, sizeof(pzone->name));

	ret = sprd_thermal_init(pzone);
	if (ret < 0) {
		dev_err(&pdev->dev,
			"cluster0 sensor sw init error id =%d\n", pzone->id);
		return ret;
	}

	sprd_ipa_info_copy(pzone, pcluster0_sensor);
	pcluster0_sensor->pzone = pzone;
	platform_set_drvdata(pdev, pzone);

	return 0;
}

static int sprd_cluster0_thm_remove(struct platform_device *pdev)
{
...
static const struct of_device_id cluster0_thermal_of_match[] = {
	{ .compatible = "sprd,cluster0-thermal" },
	{},
};
MODULE_DEVICE_TABLE(of, cluster0_thermal_of_match);

static struct platform_driver sprd_cluster0_thermal_driver = {
	.probe = sprd_cluster0_thm_probe,
	.remove = sprd_cluster0_thm_remove,
	.driver = {
		   .owner = THIS_MODULE,
		   .name = "cluster0-thermal",
		   .of_match_table = of_match_ptr(cluster0_thermal_of_match),
		   },
};

static int __init sprd_cluster0_thermal_init(void)
{
	return platform_driver_register(&sprd_cluster0_thermal_driver);
}

static void __exit sprd_cluster0_thermal_exit(void)
{
	platform_driver_unregister(&sprd_cluster0_thermal_driver);
}

device_initcall_sync(sprd_cluster0_thermal_init);
module_exit(sprd_cluster0_thermal_exit);
