...
#include "gsp_r6p0_reg.h"
#include "gsp_r6p0_coef_cal.h"
#include "../gsp_interface.h"

static void print_image_layer_cfg(struct gsp_r6p0_img_layer *layer)
{
	struct gsp_r6p0_img_layer_params *params = NULL;
...
static void print_osd_layer_cfg(struct gsp_r6p0_osd_layer *layer)
{
	struct gsp_r6p0_osd_layer_params *params = NULL;
...
static void print_des_layer_cfg(struct gsp_r6p0_des_layer *layer)
{
	struct gsp_r6p0_des_layer_params *params = NULL;
...
void gsp_r6p0_core_dump(struct gsp_core *c)
{
	...
	reg_struct.des_data_cfg.value =
		gsp_core_reg_read(R6P0_DES_DATA_CFG(c->base));
	reg_struct.des_y_addr.value =
		gsp_core_reg_read(R6P0_DES_Y_ADDR(c->base));
	reg_struct.des_u_addr.value =
		gsp_core_reg_read(R6P0_DES_U_ADDR(c->base));
	reg_struct.des_v_addr.value =
		gsp_core_reg_read(R6P0_DES_V_ADDR(c->base));
	reg_struct.des_pitch.value =
		gsp_core_reg_read(R6P0_DES_PITCH(c->base));
	reg_struct.back_rgb.value =
		gsp_core_reg_read(R6P0_BACK_RGB(c->base));
	reg_struct.work_area_size.value =
		gsp_core_reg_read(R6P0_WORK_AREA_SIZE(c->base));
	reg_struct.work_area_xy.value =
		gsp_core_reg_read(R6P0_WORK_AREA_XY(c->base));

	for (icnt = 0; icnt < R6P0_IMGL_NUM; icnt++) {
		reg_struct.limg_cfg[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_CFG(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_y_addr[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_Y_ADDR(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_u_addr[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_U_ADDR(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_v_addr[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_V_ADDR(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_pitch[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_PITCH(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_clip_start[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_CLIP_START(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_clip_size[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_CLIP_SIZE(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_des_start[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_DES_START(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_pallet_rgb[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_PALLET_RGB(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_ck[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_CK(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.y2y_y_param[icnt].value =
			gsp_core_reg_read(R6P0_Y2Y_Y_PARAM(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.y2y_u_param[icnt].value =
			gsp_core_reg_read(R6P0_Y2Y_U_PARAM(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.y2y_v_param[icnt].value =
			gsp_core_reg_read(R6P0_Y2Y_V_PARAM(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
		reg_struct.limg_des_scl_size[icnt].value =
			gsp_core_reg_read(R6P0_LIMG_DES_SIZE(
				(c->base + icnt * R6P0_LIMG_OFFSET)));
	}

	for (icnt = 0; icnt < R6P0_OSDL_NUM; icnt++) {
		reg_struct.losd_cfg[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_CFG(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
		reg_struct.losd_r_addr[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_R_ADDR(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
		reg_struct.losd_pitch[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_PITCH(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
		reg_struct.losd_clip_start[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_CLIP_START(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
		reg_struct.losd_clip_size[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_CLIP_SIZE(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
		reg_struct.losd_des_start[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_DES_START(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
		reg_struct.losd_pallet_rgb[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_PALLET_RGB(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
		reg_struct.losd_ck[icnt].value =
			gsp_core_reg_read(R6P0_LOSD_CK(
				(c->base + icnt * R6P0_LOSD_OFFSET)));
	}

	reg_struct.ip_rev.value =
		gsp_core_reg_read(R6P0_GSP_IP_REV(c->base));
	reg_struct.debug_cfg.value =
		gsp_core_reg_read(R6P0_GSP_DEBUG_CFG(c->base));
	reg_struct.debug1_cfg.value =
		gsp_core_reg_read(R6P0_GSP_DEBUG1(c->base));
...
static void gsp_r6p0_core_cfg_reinit(struct gsp_r6p0_cfg *cfg)
{
	...
	GSP_DEBUG("gsp_r6p0 cfg parent kcfg[%d]\n", gsp_kcfg_to_tag(kcfg));
	if (cfg->common.init != 1) {
		GSP_ERR("gsp_r6p0 cfg[%d] has not been initialized\n",
			cfg->common.tag);
		return;
	}
	...
	for (icnt = 0; icnt < R6P0_IMGL_NUM; icnt++)
		memset(&cfg->limg[icnt].params, 0,
			sizeof(struct gsp_r6p0_img_layer_params));
	for (icnt = 0; icnt < R6P0_OSDL_NUM; icnt++)
...
static int gsp_r6p0_core_capa_init(struct gsp_core *core)
{
	struct gsp_r6p0_capability *capa = NULL;
	struct gsp_rect max = {0, 0, 8191, 8191};
	struct gsp_rect min = {0, 0, 4, 4};

	capa = (struct gsp_r6p0_capability *)core->capa;

	/* common information initialize */
	capa->common.magic = GSP_CAPABILITY_MAGIC;
	strcpy(capa->common.version, "R6P0");
	capa->common.capa_size = sizeof(struct gsp_r6p0_capability);

	capa->common.max_layer = R6P0_IMGL_NUM + R6P0_OSDL_NUM;
	capa->common.max_img_layer = R6P0_IMGL_NUM;
...
static void gsp_r6p0_int_clear_and_disable(struct gsp_core *core)
{
...
static void gsp_r6p0_coef_cache_init(struct gsp_r6p0_core *core)
{
	...
	if (core->cache_coef_init_flag == 0) {
		...
		while (i < R6P0_GSP_COEF_CACHE_MAX) {
					...
					struct gsp_kcfg *kcfg)
{
	...
	cfg->common.layer_num = R6P0_IMGL_NUM + R6P0_OSDL_NUM;
	cfg->common.init = 1;
...
int gsp_r6p0_core_init(struct gsp_core *core)
{
	int ret = 0;
	struct gsp_kcfg *kcfg = NULL;
	struct gsp_r6p0_core *c = (struct gsp_r6p0_core *)core;
	...
	gsp_r6p0_core_capa_init(core);

	gsp_r6p0_coef_cache_init(c);

	/* to do some specific initialize operations here */
	list_for_each_entry(kcfg, &core->kcfgs, sibling) {
		gsp_r6p0_core_cfg_init((struct gsp_r6p0_cfg *)kcfg->cfg, kcfg);
	}
...
int gsp_r6p0_core_alloc(struct gsp_core **core, struct device_node *node)
{
	...
	if (IS_ERR_OR_NULL(core)) {
		GSP_ERR("r6p0 core alloc params error\n");
		return -1;
	}
	...
	if (IS_ERR_OR_NULL(pdev)) {
		GSP_ERR("find gsp_r6p0 device by child node failed\n");
		return -1;
	}
	*core = devm_kzalloc(&pdev->dev, sizeof(struct gsp_r6p0_core),
						GFP_KERNEL);
	if (IS_ERR_OR_NULL(*core)) {
	...
	memset(*core, 0, sizeof(struct gsp_r6p0_core));
	...
	if (IS_ERR_OR_NULL(capa)) {
		GSP_ERR("gsp_r6p0 capability memory allocated failed\n");
		devm_kfree(&pdev->dev, *core);
		return -1;
	}
	(*core)->capa = capa;
	(*core)->capa->capa_size = sizeof(struct gsp_r6p0_capability);
	(*core)->node = node;
	GSP_ERR("gsp_r6p0_cap_size = %d\n", (int)(*core)->capa->capa_size);
	GSP_INFO("gsp_r6p0 core allocate success\n");
	return 0;
}


static void gsp_r6p0_core_irq_enable(struct gsp_core *core)
{
	struct R6P0_GSP_INT_REG gsp_int_value;
	struct R6P0_GSP_INT_REG gsp_int_mask;
...
int gsp_r6p0_core_enable(struct gsp_core *c)
{
	int ret = -1;
...
void gsp_r6p0_core_disable(struct gsp_core *c)
{
	struct gsp_r6p0_core *core = NULL;
...
static int gsp_r6p0_core_parse_clk(struct gsp_r6p0_core *core)
{
	int status = 0;
...
static int gsp_r6p0_core_parse_irq(struct gsp_core *core)
{
	int ret = -1;
	struct device *dev = NULL;
	static const char * const r6p0_irq_name[] = {
		"GSP0", "GSP1"
	};
	...
	if (core->irq)
		ret = devm_request_irq(dev, core->irq,
				gsp_r6p0_core_irq_handler,
				IRQF_SHARED, r6p0_irq_name[core->id], core);
...
int gsp_r6p0_core_parse_dt(struct gsp_core *core)
{
	...
	r6p0_core = (struct gsp_r6p0_core *)core;

	r6p0_core->gsp_ctl_reg_base = core->base;
	GSP_INFO("gsp_ctl_reg_base: 0x%p\n", core->base);

	ret = gsp_r6p0_core_parse_irq(core);
	if (ret) {
		GSP_ERR("core[%d] parse irq failed\n", core->id);
		return ret;
	}

	gsp_r6p0_core_parse_clk(r6p0_core);
			...
			struct gsp_r6p0_cfg *cfg)
{
	...
	struct R6P0_WORK_AREA_XY_REG work_area_xy_mask;
	struct R6P0_WORK_AREA_SIZE_REG work_area_size_value;
	struct R6P0_WORK_AREA_SIZE_REG work_area_size_mask;
	struct R6P0_GSP_MOD_CFG_REG gsp_mod_cfg_value;
	struct R6P0_GSP_MOD_CFG_REG gsp_mod_cfg_mask;
	...
	gsp_core_reg_update(R6P0_GSP_MOD_CFG(base),
	...
	gsp_core_reg_update(R6P0_WORK_AREA_XY(base),
	...
	gsp_core_reg_update(R6P0_WORK_AREA_SIZE(base),
		work_area_size_value.value, work_area_size_mask.value);
}

static void gsp_r6p0_core_limg_reg_set(void __iomem *base,
			   struct gsp_r6p0_img_layer *layer, int layer_index)
{
	struct R6P0_LAYERIMG_CFG_REG limg_cfg_value;
	struct R6P0_LAYERIMG_CFG_REG limg_cfg_mask;
	struct R6P0_LAYERIMG_Y_ADDR_REG limg_y_addr_value;
	struct R6P0_LAYERIMG_Y_ADDR_REG limg_y_addr_mask;
	struct R6P0_LAYERIMG_U_ADDR_REG limg_u_addr_value;
	struct R6P0_LAYERIMG_U_ADDR_REG limg_u_addr_mask;
	struct R6P0_LAYERIMG_V_ADDR_REG limg_v_addr_value;
	struct R6P0_LAYERIMG_V_ADDR_REG limg_v_addr_mask;
	struct R6P0_LAYERIMG_PITCH_REG limg_pitch_value;
	struct R6P0_LAYERIMG_PITCH_REG limg_pitch_mask;
	struct R6P0_LAYERIMG_CLIP_SIZE_REG limg_clip_size_value;
	struct R6P0_LAYERIMG_CLIP_SIZE_REG limg_clip_size_mask;
	struct R6P0_LAYERIMG_CLIP_START_REG limg_clip_start_value;
	struct R6P0_LAYERIMG_CLIP_START_REG limg_clip_start_mask;
	struct R6P0_LAYERIMG_DES_START_REG limg_des_start_value;
	struct R6P0_LAYERIMG_DES_START_REG limg_des_start_mask;
	struct R6P0_LAYERIMG_CK_REG limg_ck_value;
	struct R6P0_LAYERIMG_CK_REG limg_ck_mask;
	struct R6P0_LAYERIMG_DES_SCL_SIZE_REG limg_des_size_value;
	struct R6P0_LAYERIMG_DES_SCL_SIZE_REG limg_des_size_mask;
	struct gsp_r6p0_img_layer_params *limg_params = NULL;
	...
	if (layer->common.enable != 1) {
		...
		gsp_core_reg_update(R6P0_LIMG_CFG(
			(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LIMG_Y_ADDR(
		(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LIMG_PITCH(
		(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LIMG_CLIP_SIZE(
		(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LIMG_CLIP_START(
		(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LIMG_DES_START(
		(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LIMG_DES_SIZE(
		(base + layer_index * R6P0_LIMG_OFFSET)),
		limg_des_size_value.value, limg_des_size_mask.value);

	/* limg pallet rgb set */
	if (limg_params->pallet_en) {
		struct R6P0_LAYERIMG_PALLET_RGB_REG limg_pallet_value;
		struct R6P0_LAYERIMG_PALLET_RGB_REG limg_pallet_mask;
		...
		gsp_core_reg_update(R6P0_LIMG_PALLET_RGB(
			(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LIMG_CK(
		(base + layer_index * R6P0_LIMG_OFFSET)),
	...
	if (limg_params->y2y_mod == 0) {
		struct R6P0_Y2Y_Y_PARAM_REG y2y_y_para_value;
		struct R6P0_Y2Y_Y_PARAM_REG y2y_y_para_mask;
		struct R6P0_Y2Y_U_PARAM_REG y2y_u_para_value;
		struct R6P0_Y2Y_U_PARAM_REG y2y_u_para_mask;
		struct R6P0_Y2Y_V_PARAM_REG y2y_v_para_value;
		struct R6P0_Y2Y_V_PARAM_REG y2y_v_para_mask;
		...
		gsp_core_reg_update(R6P0_Y2Y_Y_PARAM(
			(base + layer_index * R6P0_LIMG_OFFSET)),
		...
		gsp_core_reg_update(R6P0_Y2Y_U_PARAM(
			(base + layer_index * R6P0_LIMG_OFFSET)),
		...
		gsp_core_reg_update(R6P0_Y2Y_V_PARAM(
			(base + layer_index * R6P0_LIMG_OFFSET)),
...
static void gsp_r6p0_core_losd_reg_set(void __iomem *base,
			   struct gsp_r6p0_osd_layer *layer, int layer_index)
{
	struct R6P0_LAYEROSD_CFG_REG losd_cfg_value;
	struct R6P0_LAYEROSD_CFG_REG losd_cfg_mask;
	struct R6P0_LAYEROSD_R_ADDR_REG losd_r_addr_value;
	struct R6P0_LAYEROSD_R_ADDR_REG losd_r_addr_mask;
	struct R6P0_LAYEROSD_PITCH_REG losd_pitch_value;
	struct R6P0_LAYEROSD_PITCH_REG losd_pitch_mask;
	struct R6P0_LAYEROSD_CLIP_SIZE_REG losd_clip_size_value;
	struct R6P0_LAYEROSD_CLIP_SIZE_REG losd_clip_size_mask;
	struct R6P0_LAYEROSD_CLIP_START_REG losd_clip_start_value;
	struct R6P0_LAYEROSD_CLIP_START_REG losd_clip_start_mask;
	struct R6P0_LAYEROSD_DES_START_REG losd_des_start_value;
	struct R6P0_LAYEROSD_DES_START_REG losd_des_start_mask;
	struct R6P0_LAYEROSD_CK_REG losd_ck_value;
	struct R6P0_LAYEROSD_CK_REG losd_ck_mask;
	struct gsp_r6p0_osd_layer_params *losd_params = NULL;
	...
	if (layer->common.enable != 1) {
		...
		gsp_core_reg_update(R6P0_LOSD_CFG(
	...
	gsp_core_reg_update(R6P0_LOSD_PITCH(
		(base + layer_index * R6P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LOSD_CLIP_START(
		(base + layer_index * R6P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LOSD_CLIP_SIZE(
		(base + layer_index * R6P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LOSD_DES_START(
		(base + layer_index * R6P0_LOSD_OFFSET)),
		losd_des_start_value.value, losd_des_start_mask.value);

	/* losd pallet set */
	if (losd_params->pallet_en) {
		struct R6P0_LAYEROSD_PALLET_RGB_REG losd_pallet_value;
		struct R6P0_LAYEROSD_PALLET_RGB_REG losd_pallet_mask;
		...
		gsp_core_reg_update(R6P0_LOSD_PALLET_RGB(
			(base + layer_index * R6P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R6P0_LOSD_CK(
		(base + layer_index * R6P0_LOSD_OFFSET)),
			   ...
			   struct gsp_r6p0_des_layer *layer)
{
	...
	struct R6P0_DES_Y_ADDR_REG des_y_addr_value;
	struct R6P0_DES_Y_ADDR_REG des_y_addr_mask;
	struct R6P0_DES_U_ADDR_REG des_u_addr_value;
	struct R6P0_DES_U_ADDR_REG des_u_addr_mask;
	struct R6P0_DES_V_ADDR_REG des_v_addr_value;
	struct R6P0_DES_V_ADDR_REG des_v_addr_mask;
	struct R6P0_DES_PITCH_REG des_pitch_value;
	struct R6P0_DES_PITCH_REG des_pitch_mask;
	struct gsp_r6p0_des_layer_params *ld1_params = NULL;
	...
	if (ld1_params->bk_para.bk_enable) {
		...
		struct R6P0_BACK_RGB_REG back_rgb_mask;
		...
		gsp_core_reg_update(R6P0_BACK_RGB(base),
						...
						struct gsp_r6p0_cfg *cmd)
{
	...
	for (jcnt = 0; jcnt < R6P0_IMGL_NUM; jcnt++)
		if (cmd->limg[jcnt].params.scaling_en) {
			...
			ret_coef = gsp_r6p0_gen_block_scaler_coef(core,
								src_w, src_h,
								dst_w, dst_h,
								hor_tap,
								ver_tap);
		 ...
		 */
			if (ret_coef) {
				for (i = 0; i < 128; i++)
					gsp_core_reg_update(
						R6P0_SCALE_COEF_ADDR(
						(core->gsp_ctl_reg_base +
						jcnt * R6P0_SCALE_COEF_OFFSET
						+ i * 4)),
						*ret_coef++, 0xffffffff);
...
static void gsp_r6p0_core_run(struct gsp_core *core)
{
	struct R6P0_GSP_GLB_CFG_REG gsp_mod1_cfg_value;
	struct R6P0_GSP_GLB_CFG_REG gsp_mod1_cfg_mask;

	gsp_mod1_cfg_value.value = 0;
	gsp_mod1_cfg_value.GSP_RUN0 = 1;
	gsp_mod1_cfg_mask.value = 0;
	gsp_mod1_cfg_mask.GSP_RUN0 = 0x1;
	gsp_core_reg_update(R6P0_GSP_GLB_CFG(core->base),
		gsp_mod1_cfg_value.value, gsp_mod1_cfg_mask.value);
}

static int gsp_r6p0_core_run_precheck(struct gsp_core *c)
{
	return 0;
}
int gsp_r6p0_core_trigger(struct gsp_core *c)
{
	...
	struct gsp_r6p0_core *core = (struct gsp_r6p0_core *)c;
	struct R6P0_GSP_GLB_CFG_REG gsp_mod1_cfg_value;

	if (gsp_core_verify(c)) {
		GSP_ERR("gsp_r6p0 core trigger params error\n");
		return ret;
	}

	kcfg = c->current_kcfg;
	if (gsp_kcfg_verify(kcfg)) {
		GSP_ERR("gsp_r6p0 trigger invalidate kcfg\n");
		return ret;
	}
	...
	base = c->base;
	cfg = (struct gsp_r6p0_cfg *)kcfg->cfg;

	gsp_r6p0_coef_gen_and_cfg(core, cfg);

	gsp_r6p0_core_misc_reg_set(c, cfg);
	for (icnt = 0; icnt < R6P0_IMGL_NUM; icnt++)
		gsp_r6p0_core_limg_reg_set(base, &cfg->limg[icnt], icnt);
	for (icnt = 0; icnt < R6P0_OSDL_NUM; icnt++)
		gsp_r6p0_core_losd_reg_set(base, &cfg->losd[icnt], icnt);
	gsp_r6p0_core_ld1_reg_set(base, &cfg->ld1);

	if (gsp_r6p0_core_run_precheck(c)) {
		GSP_ERR("r6p0 core run precheck fail !\n");
...
{
	struct gsp_r6p0_cfg_user  __user *cfgs = NULL;

	cfgs = (struct gsp_r6p0_cfg_user __user *)arg;
