 ...
 */
static struct COEF_ENTRY_T *gsp_r7p0_coef_cache_hit_check(
		struct gsp_r7p0_core *core,
...
static inline void gsp_r7p0_coef_cache_move_to_head(
					struct gsp_r7p0_core *core,
...
uint32_t *gsp_r7p0_gen_block_scaler_coef(struct gsp_r7p0_core *core,
				 uint32_t in_sz_x,
				 uint32_t in_sz_y,
				 uint32_t ouf_sz_x,
				 uint32_t ouf_sz_y,
				 ...
				 uint32_t ver_tap)
{
	...
	if (core->cache_coef_init_flag == 1) {
		entry = gsp_r7p0_coef_cache_hit_check(core,
			in_sz_x, in_sz_y, ouf_sz_x, ouf_sz_y, hor_tap, ver_tap);
		if (entry) {
			gsp_r7p0_coef_cache_move_to_head(core, entry);
			return entry->coef;
		}
	}
