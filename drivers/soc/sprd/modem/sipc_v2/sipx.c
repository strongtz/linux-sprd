...
/*
 * TODO:
 * because in orca, we use the IPA module repalce sipx,
 * so we don't support client mode in sipx. if we don't use IPA module,
 * we need add client mode codes in sipx for orca
 */
...
static int sipx_ul_test_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (g_flow_on) {
			...
			SIPX_ERR("%s sent %d pkts\n", __func__, 40 * 5);
			usleep_range(1000, 1500);
			SIPX_ERR("%s sent start\n", __func__);
		} else {
			msleep(100);
		}
	}
...
int sipx_chan_create(u8 dst, u8 channel)
{
	...
	if (ret) {
		SIPX_ERR("Failed to create channel, ret = %d", ret);
		goto fail;
	}
...
static int sipx_debug_show(struct seq_file *m, void *private)
{
	...
	for (i = 0; i < SIPC_ID_NR; i++) {
		...
		seq_printf(m, "smem_addr: 0x%0x, dst_smem_addr:",
			   sipx->smem_addr);
		seq_printf(m, "0x%0x\n, smem_size: 0x%0x",
			   sipx->dst_smem_addr,
			   sipx->smem_size);

		seq_printf(m, "smem_cached_virt: 0x%p, dst_smem_addr: 0x%0x\n",
