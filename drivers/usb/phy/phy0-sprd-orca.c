...
static void sprd_ssphy_reset_core(struct sprd_ssphy *phy)
{
	...
	reg = msk = MASK_AP_AHB_RF_USB0_SOFT_RST;
	regmap_update_bits(phy->ap_ahb, REG_AP_AHB_RF_AHB_RST, msk, reg);
	...
	reg = msk = MASK_AP_AHB_RF_USB0_SOFT_RST;
	regmap_update_bits(phy->ap_ahb, REG_AP_AHB_RF_AHB_RST, msk, 0);
}
...
static int sprd_ssphy_init(struct usb_phy *x)
{
	...
	if (atomic_read(&phy->inited)) {
		dev_info(x->dev, "ssphy is already inited!\n");
		return 0;
	}
	...
	ret |= regmap_read(phy->ana_g4,
		REG_ANLG_PHY_G4_RF_ANALOG_USB3_TYPEC_0_ANA_USB30_CTRL1, &reg);
	msk = MASK_ANLG_PHY_G4_RF_ANALOG_USB3_TYPEC_0_USB30_PS_PD_S |
		MASK_ANLG_PHY_G4_RF_ANALOG_USB3_TYPEC_0_USB30_PS_PD_L |
		MASK_ANLG_PHY_G4_RF_ANALOG_USB3_TYPEC_0_USB30_ISO_SW_EN;
	reg &= ~msk;
	...
	/* USB2 PHY power on */
	ret |= regmap_read(phy->ana_g4,
		REG_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_BATTER_PLL, &reg);
	msk = MASK_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_PS_PD_S |
		MASK_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_PS_PD_L;
	reg &= ~msk;
	ret |= regmap_write(phy->ana_g4,
		REG_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_BATTER_PLL, reg);
	...
	reg = msk = MASK_AP_AHB_RF_UTMISRP_BVALID_REG0 |
		MASK_AP_AHB_RF_OTG_VBUS_VALID_PHYREG0 |
		MASK_AP_AHB_RF_PIPE3_POWERPRESENT0;
	ret |= regmap_update_bits(phy->ap_ahb, REG_AP_AHB_RF_USB0_CTRL,
				  msk, reg);

	msk = MASK_AP_AHB_RF_OTG_VBUS_VALID_PHYREG_SEL0;
	ret |= regmap_update_bits(phy->ap_ahb, REG_AP_AHB_RF_USB0_CTRL,
				  msk, 0);

	/* disable power management event */
	msk = MASK_AP_AHB_RF_PME_EN0;
	ret |= regmap_update_bits(phy->ap_ahb, REG_AP_AHB_RF_USB0_CTRL,
				  msk, 0);
	...
	ret |= regmap_update_bits(phy->ana_g4,
			REG_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_UTMI_CTL1,
				  msk, reg);
...
static void sprd_ssphy_shutdown(struct usb_phy *x)
{
	...
	msk = MASK_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_VBUSVLDEXT;
	regmap_update_bits(phy->ana_g4,
		REG_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_UTMI_CTL1, msk, 0);

	/* purpose: set bvalid vbus_valid and clear vbus_valid_sel */
	msk = MASK_AP_AHB_RF_UTMISRP_BVALID_REG0 |
		MASK_AP_AHB_RF_OTG_VBUS_VALID_PHYREG0 |
		MASK_AP_AHB_RF_PIPE3_POWERPRESENT0;
	regmap_update_bits(phy->ap_ahb, REG_AP_AHB_RF_USB0_CTRL, msk, 0);
				  ...
				  unsigned long event, void *data)
{
	...
	if (event) {
		...
		reg = msk =
		MASK_ANLG_PHY_G4_RF_DBG_SEL_ANALOG_USB20_0_USB20_DPPULLDOWN |
		MASK_ANLG_PHY_G4_RF_DBG_SEL_ANALOG_USB20_0_USB20_DMPULLDOWN;
		regmap_update_bits(ssphy->ana_g4,
				REG_ANLG_PHY_G4_RF_ANALOG_USB20_0_REG_SEL_CFG_0,
				   msk, reg);
		...
		regmap_update_bits(ssphy->ana_g4,
			   REG_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_UTMI_CTL2,
				   msk, reg);

		reg = 0x200;
		msk = MASK_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_RESERVED;
		regmap_update_bits(ssphy->ana_g4,
			REG_ANLG_PHY_G4_RF_ANALOG_USB20_0_USB20_UTMI_CTL1,
				   msk, reg);
	} else {
