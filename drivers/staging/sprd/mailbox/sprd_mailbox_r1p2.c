...
static bool mbox_phy_outbox_has_irq(void)
{
	...
	return (irq_sts & MBOX_IRQ_CLR_BIT);
}
...
static const struct mbox_operations_tag mbox_r1p2_operation = {
...
static struct mbox_device_tag mbox_r1p2_device = {
	.version = 0x102,
	.max_cnt = MBOX_MAX_CORE_CNT,
	.fops = &mbox_r1p2_operation,
#if defined(CONFIG_DEBUG_FS)
...
static int mbox_cfg_init(struct mbox_dts_cfg_tag *mbox_dts_cfg, u8 *mbox_inited)
{
	...
	sprd_inbox_base = (unsigned long)ioremap_nocache(mbox_dts_cfg->inboxres.start,
	...
	/* init outbox base */
	mbox_cfg.outbox_base = mbox_dts_cfg->outboxres.start;
	sprd_outbox_base = (unsigned long)ioremap_nocache(mbox_dts_cfg->outboxres.start,
							  resource_size(&mbox_dts_cfg->outboxres));
	if (!sprd_outbox_base) {
		pr_err("mbox:ERR on line %d!\n", __LINE__);
		return -EINVAL;
	}
