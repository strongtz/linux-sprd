...
static struct pasr_info pasr_info __initdata;
	...
	for (i = 0; i < map->nr_dies; i++) {
		...
		for (j = 0; j < die->nr_sections; j++) {
			...
			seq_printf(s, "section %d %lu %llu\n"
				, j, section->free_size, percentage);
		}
	}
	return 0;
}
...
static int __init pasr_init_debug(void)
{
	...
	d = debugfs_create_file("meminfo", 0444, rootdir, (void *)&pasr_map,
				&meminfo_fops);
	if (!d)
		return -ENOMEM;
...
int __init late_pasr_setup(void)
{
	...
	for (i = 0; i < pasr_map.nr_dies; i++) {
			for (s = &pasr_map.die[i].section[0], j = 0
			; j < pasr_map.die[i].nr_sections
			; j++, s = &pasr_map.die[i].section[j]) {
				if (!s->lock) {
					s->lock = kzalloc(sizeof(spinlock_t)
						, GFP_KERNEL);
					BUG_ON(!s->lock);
