...
static void dump_sdio_reg(struct sprd_sdhc_host *host)
{
	...
	print_hex_dump(KERN_ERR, "sprd-sdhcr11", DUMP_PREFIX_OFFSET,
...
static void sprd_set_delay_value(struct sprd_sdhc_host *host)
{
	...
	case MMC_TIMING_LEGACY:
		if (host->ios.clock == 25000000) {
			...
			dev_info(dev,
				 "legacy default timing delay value 0x%08x\n",
				 host->dll_dly);
		}
		break;
	...
	case MMC_TIMING_SD_HS:
		if (host->ios.clock == 50000000) {
			...
			sprd_sdhc_writel(host, host->dll_dly,
					SPRD_SDHC_REG_32_DLL_DLY);
			dev_info(dev,
				 "sdhs default timing delay value 0x%08x\n",
				 host->dll_dly);
		}
		break;
		...
		struct sprd_sdhc_host *host)
{
	...
	host->clk_source = devm_clk_get(dev, "sdio_clk_source");
	...
	host->sdio_ahb = devm_clk_get(dev, "sdio_ahb_enable");
	...
	host->sdio_hp_det_clk = devm_clk_get(dev, "sdio_hp_det_clk");
	if (IS_ERR(host->sdio_hp_det_clk)) {
		dev_warn(dev, "Can't get clock property: sdio_hp_det_clk\n");
		host->sdio_hp_det_clk = NULL;
	}

	host->sdio_ckg = devm_clk_get(dev, "sdio_2x_eb");
	if (IS_ERR(host->sdio_ckg)) {
		dev_err(dev, "Can't get clock property: sdio_2x_eb\n");
		host->sdio_ckg = NULL;
	}

	host->sdio_1x_ckg = devm_clk_get(dev, "sdio_1x_eb");
...
void sprd_sdhc_restore_dly(struct sprd_sdhc_host *host)
{
	...
	sprd_sdhc_writel(host, host->dll_dly, SPRD_SDHC_REG_32_DLL_DLY);
	dev_info(dev, "dll delay reg: 0x%x\n",
		 sprd_sdhc_readl(host, SPRD_SDHC_REG_32_DLL_DLY));
}
